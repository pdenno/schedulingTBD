...
(def proj-summary-msgs
  (conj domain/project-name-prompt ; This asks for 3 or less words; that requirement will often be ignored by gpt-3.5-turbo.
                       {:role "user"
                        :content
...
(deftest chat2clj-tests
  (testing "Testing a simple query with different LLMs."
    (is (let [res (chat2clj proj-summary-msgs {:model "gpt-4"})]
          (and (map? res)
               (= (keys res) '(:summary))
               (-> res :summary (str/split #"\s+") count (<= 3)))))
    (is (let [res (chat2clj proj-summary-msgs {:model "gpt-3.5-turbo"})]
            (and (map? res)
                 (= (keys res) '(:summary))
