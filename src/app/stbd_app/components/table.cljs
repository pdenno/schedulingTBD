...
;;; FIXED 2025-07-05: Cursor positioning issue - see docs/fixes/table-editcell-cursor-fix.md
(defnc EditCell [{:keys [cell-id row-id text table set-table-fn]}]
  (let [[local-text set-local-text] (hooks/use-state text)]
    ($ TableCell {:key cell-id :sx #js {:padding "0"}}
       ($ TextField {:key (str row-id "-" cell-id) ; Use stable key based on position, not content
                     :sx #js {:bgcolor "#fff6d9" :alignItems "stretch" :width "100%"} ; "#f0e699" is the yellow color used in MessageList; this is lighter.
                     :value local-text ; Use controlled component with local state
                     :autoFocus true
                     :onChange (fn [event]
                                 (let [val (j/get-in event [:target :value])]
                                   (set-local-text val) ; Update local state immediately for responsive typing
                                   (set-table-fn ; Update table state
  ...
  (let [index-id (atom 0)
    ...
    (for [table-row (:table-body table)]
      (let [ix (swap! index-id inc)
        ...
        ($ TableRow {:key row-id :editMode "row" #_#_:editable true :sx #js {:width "100%" :minWidth "1300px" :alignItems "stretch" :padding "0 0 0 0"}}
           (-> (for [k column-keys]
    ...
    (letfn [(handle-open [] (when (j/get modal :current) (set-open true)))
      ...
      ($ Box {:ref modal}
         ...
         ($ Dialog {:open open :onClose handle-close
                    :sx #js {:width "fitContent" :maxWidth "xl" :fullWidth true #_#_:minWidth "1400px"}}
