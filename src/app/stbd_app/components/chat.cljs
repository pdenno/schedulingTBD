...
(defnc Chat [{:keys [chat-height conv-map proj-info active-conv-in] :or {active-conv-in :process}}]
  (let [[msg-list set-msg-list]         (hooks/use-state (:conv conv-map))
        ...
        [active-conv set-active-conv]   (hooks/use-state active-conv-in)
        resize-fns (make-resize-fns set-box-height)]
    (letfn [(change-conversation-click [to]
              (if-let [pid (:project/id @ws/project-info)]
                (do (ws/send-msg {:dispatch-key :change-conversation :pid pid :conv-id to})
